...
export const VideoUpload = ({ label, value, onChange, onFileSelect, onMetadataExtracted, required, description, uploadImmediately = true }: VideoUploadProps) => {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const validation = validate(file, 'video');
    if (!validation.valid) {
    ...
    try {
      ...
      if (uploadImmediately) {
        toast({ title: "Subiendo video...", description: "Este proceso puede tardar unos minutos dependiendo del tamaño del archivo." });

        const result = await uploadFile(file, { mediaType: 'video' });
      ...
      try {
        ...
        videoEl.onloadedmetadata = () => {
          const metadata = { thumbnail: '', width: videoEl.videoWidth, height: videoEl.videoHeight, size: file.size, duration: videoEl.duration };
          ...
          videoEl.onseeked = () => {
            ...
            try {
              ...
              if (ctx) { ctx.drawImage(videoEl, 0, 0); metadata.thumbnail = canvas.toDataURL('image/jpeg', 0.8); }
            } catch (_) { /* thumbnail failed */ }
            if (onMetadataExtracted) onMetadataExtracted(metadata);
            URL.revokeObjectURL(metadataUrl);
            setLoading(false);
          };
        };
        videoEl.onerror = () => { clearTimeout(metadataTimeout); URL.revokeObjectURL(metadataUrl); setLoading(false); };
      } catch (_) { setLoading(false); }
    } catch (error) {
      setLoading(false);
      toast({ title: "Error", description: (error as Error).message || "No se pudo cargar el video", variant: "destructive" });
    }
  };
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex flex-col gap-4">
        {preview ? (
          <div className="relative w-full max-w-md rounded-lg overflow-hidden border-2 border-border">
            {videoPlaybackError ? (
              ...
              <video ref={videoRef} src={preview} controls playsInline className="w-full h-auto max-h-64 object-contain bg-black" onError={() => setVideoPlaybackError(true)} />
            )}
            <button type="button" onClick={handleRemove} className="absolute top-2 right-2 p-1 bg-destructive text-destructive-foreground rounded-full hover:bg-destructive/90 transition-colors">
              <X className="w-4 h-4" />
            </button>
          </div>
        ) : (
          <div onClick={() => fileInputRef.current?.click()} className="flex flex-col items-center justify-center w-full max-w-md h-40 border-2 border-dashed border-border rounded-lg cursor-pointer hover:border-primary/50 transition-colors bg-muted/20">
            {loading ? (
        ...
        <Input ref={fileInputRef} type="file" accept="video/mp4,video/webm,video/quicktime,video/x-matroska,.mp4,.webm,.mov,.mkv,.avi" onChange={handleFileChange} className="hidden" disabled={loading} />
      </div>

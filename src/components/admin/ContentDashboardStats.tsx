...
import { Clock, CheckCircle2, XCircle, TrendingUp, BarChart3, Video, AlertCircle, Download, FileSpreadsheet } from "lucide-react";
...
export function ContentDashboardStats() {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      setStats({ pending, approved, rejected, total: contentData?.length || 0 });

      const typeStats: Record<string, ContentByType> = {};
      contentData?.forEach(content => {
        if (!typeStats[content.content_type]) {
          typeStats[content.content_type] = { type: content.content_type, pending: 0, approved: 0, rejected: 0 };
        }
      ...
      const recent = contentData
        ...
        .slice(0, 5)
        .map(c => ({ id: c.id, title: c.title, content_type: c.content_type, status: c.status, created_at: c.created_at })) || [];
  ...
  const handleExportDashboard = async () => {
    try {
      ...
      const contentByTypeData = contentByType.map(ct => ({
        ...
        'Pendiente': ct.pending, 'Aprobado': ct.approved, 'Rechazado': ct.rejected,
      ...
      const contentListData = allContent.map(c => ({
        ...
        'Vistas': c.views_count || 0, 'Likes': c.likes_count || 0,
        'Fecha de Subida': formatDate(c.created_at),
      }));

      const usersByType = Object.entries(
        (usersRes.data || []).reduce((acc, u) => { acc[u.profile_type] = (acc[u.profile_type] || 0) + 1; return acc; }, {} as Record<string, number>)
      ).map(([type, count]) => ({ 'Tipo de Perfil': PROFILE_TYPE_LABELS[type] || type, 'Cantidad': count }));

      const usersByCountry = Object.entries(
        (usersRes.data || []).reduce((acc, u) => { acc[u.pais] = (acc[u.pais] || 0) + 1; return acc; }, {} as Record<string, number>)
      ).sort((a, b) => b[1] - a[1]).map(([country, count]) => ({ 'País': country, 'Cantidad': count }));
      ...
      toast({ title: "Exportación exitosa", description: "Se descargó el archivo Excel con todas las estadísticas" });
    } catch (error) {
  ...
  const getContentTypeLabel = (type: string) => {
    const labels: Record<string, string> = {
      video_musical_vivo: "Video Musical en Vivo", video_clip: "Video Clip", podcast: "Podcast",
      corto: "Cortometraje", documental: "Documental", pelicula: "Película"
    };
  ...
  return (
    <div className="space-y-6">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="bg-card border-border">
            <DropdownMenuItem onClick={handleExportDashboard}>
              <FileSpreadsheet className="w-4 h-4 mr-2" />Excel Completo (.xlsx)
            </DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card className="border-green-500/20 bg-gradient-to-br from-green-500/5 to-emerald-500/5">
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-1">{approvalRate.toFixed(1)}% del total</p>
          </CardContent>
        </Card>
        <Card className="border-red-500/20 bg-gradient-to-br from-red-500/5 to-rose-500/5">
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-1">{rejectionRate.toFixed(1)}% del total</p>
          </CardContent>
        </Card>
        <Card className="border-primary/20 bg-gradient-to-br from-primary/5 to-purple-500/5">
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-1">Contenido subido</p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2"><TrendingUp className="h-5 w-5" />Distribución por Estado</CardTitle>
            <CardDescription>Proporción de contenido en cada estado</CardDescription>
          </CardHeader>
          <CardContent>
            {pieData.length > 0 ? (
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2"><BarChart3 className="h-5 w-5" />Contenido por Tipo</CardTitle>
            <CardDescription>Distribución por tipo de contenido y estado</CardDescription>
          </CardHeader>
          <CardContent>
            {contentByType.length > 0 ? (
              <ResponsiveContainer width="100%" height={300}>
                <BarChart data={contentByType}>
                  ...
                  <XAxis dataKey="type" angle={-45} textAnchor="end" height={100} tick={{ fontSize: 10 }} />
                  <YAxis />
                  <Tooltip />
                  <Legend />
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Últimas 5 subidas de contenido</CardDescription>
        </CardHeader>
        <CardContent>
          {recentContent.length > 0 ? (
            ...
            <div className="text-center py-8 text-muted-foreground">No hay contenido reciente</div>
          )}
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Porcentaje de contenido aprobado vs rechazado</CardDescription>
        </CardHeader>
